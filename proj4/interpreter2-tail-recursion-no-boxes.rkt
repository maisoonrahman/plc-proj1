...
(define test
  (lambda ()
    (scheme->language
     (get-overall-env "Input.rkt"))))

(define get-overall-env
  (lambda (file)
    (interpret-statement-list (parser file) (list (add-but-funky 'return '() (newframe))) (lambda (v) v)
...
(define interpret-statement
  (lambda (statement environment return break continue throw next)
    (cond
      ...
      ((eq? 'class (statement-type statement)) (interpret-class-creation statement environment return next))
      (else (myerror "Unknown statement:" (statement-type statement))))))

; Calls the return continuation with the given expression value
(define interpret-return
  (lambda (statement environment return)
    (return (set-return (eval-expression (get-expr statement) environment) environment))))
...
(define create-method-closures
  (lambda (statement-list)
    (cond
      ((null? statement-list) (newframe))
      ((eq? 'function (caar statement-list)) (add-but-funky (cadar statement-list) (create-single-normal-method-closure (car statement-list)) (create-method-closures (cdr statement-list))))
...
(define get-super-closure operator)
(define get-class-fields operand1)
(define get-class-methods operand2)
